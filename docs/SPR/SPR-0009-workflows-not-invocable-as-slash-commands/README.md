...
| 7 | **Report an upstream defect in DAR-OW-096** | Action | Its synthesis file (`03_Synthesis/00_Containerized_Amnesiac_Deployment_Synthesis.md`) carries frontmatter for a different document: `type: DAR.SYNTHESIS`, `status: ACTIVE`, title `sc_verifier uses raw DML via get_writer_conn — TP Change Report`. The governed record for its id 14925 says `doc_type: SPR.TP_CHANGE`, `status: CLOSED`, `parent_ci: 14922`. The file and its registration disagree. | found by `query_ci_by_id --ci-id 14925` |
